...
class CharSheet(Frame):
    ...
    def __init__(self, *args, sb_width=20, **kwargs):
        ...
        self.content = Scrollable(self, width=sb_width)
        self.parent = self.winfo_parent()
        ...
        # Update scrollable once everything is packed
        self.content.update()

        # Make all LabelFrame boxes equal width
        self.content.bind('<Configure>', self.__on_configure, add='+')
    ...
    def __on_configure(self, event):
        self.after(100, self.__equalize_boxwidths, event)

    def __equalize_boxwidths(self, event):
        """Make all LabelFrames (sections) equal in width"""
        print('event triggered')
        print(event.widget)
        # 1) Get list of all descendants in scrollable frame
        descs = gt.find_all_descendants(self.content, 3)
        lfs = list()
        # 2) Iterate through all LabelFrame objects, scanning for max width.
        # (Discard non-LabelFrame items from list)
        w = list()
        for desc in descs:
            if desc.winfo_class() == 'Labelframe':
                w.append(desc.winfo_width() +
                         float(str(desc.cget('padx')))*2)
                lfs.append(desc)
        print(w)
        w_max = max(w)

        # 3) Iterate through again, setting all widths equal to max width.
        for lf in lfs:
            total_width = lf.winfo_width() + float(str(lf.cget('padx')))*2
            if total_width < w_max - 5:
                print(lf)
                print('curr: ' + str(total_width))
                print('max: ' + str(w_max))
                lf.config(padx=(w_max - lf.winfo_width())/2)
                total_width = lf.winfo_width() + float(str(lf.cget('padx')))*2
                print('new: ' + str(total_width))
                w = list()
                for lf in lfs:
                    w.append(lf.winfo_width() + float(str(lf.cget('padx')))*2)
                w_max = max(w)

        # 4) Temporary for debugging: print "after" vals
        w = list()
        for lf in lfs:
            w.append(lf.winfo_width() + float(str(lf.cget('padx')))*2)
        print(w)
...
class GeneralInfoForm(Frame):
    ...
    def __init__(self, master=None, *args, padx_ext=8, pady_ext=8, **kwargs):
        ...
        self.label_frame.pack(expand=False, fill=Y)
...
class AttributesForm(Frame):
    ...
    def __init__(self, master=None, cnf={}, *args, padx_ext=8, pady_ext=8, **kwargs):
        ...
        self.label_frame.pack(fill=Y, expand=False)
